...
describe("findStalePendingOrders", () => {
  it("returns a pending order created more than thresholdDays before now (AC1)", async () => {
    const db = await openTestDb();
    const now = new Date("2026-07-19T00:00:00.000Z");

    const customerId = await seedCustomer(db, {
      email: "stale-buyer@example.com",
      firstName: "Stale",
      lastName: "Buyer",
      phone: "555-1234",
    });

    const staleOrderId = await seedOrder(db, {
      customerId,
      status: "pending",
      createdAt: toSqliteTimestamp(daysBefore(now, 10)),
    });

    const results = await findStalePendingOrders(db, 3, now);

    expect(results).toHaveLength(1);
    expect(results[0].order_id).toBe(staleOrderId);
    expect(results[0].customer_id).toBe(customerId);
    expect(results[0].customer_row_id).toBe(customerId);
    expect(results[0].first_name).toBe("Stale");
    expect(results[0].last_name).toBe("Buyer");
    expect(results[0].phone).toBe("555-1234");

    await db.close();
  });

  it("excludes a pending order created less than thresholdDays before now (AC2)", async () => {
    const db = await openTestDb();
    const now = new Date("2026-07-19T00:00:00.000Z");
  ...
  it("excludes a processing order of any age (AC3)", async () => {
    const db = await openTestDb();
    const now = new Date("2026-07-19T00:00:00.000Z");
  ...
  it("surfaces a stale order whose customer_id has no matching customers row, with customer_row_id null but customer_id still populated", async () => {
    const db = await openTestDb();
    const now = new Date("2026-07-19T00:00:00.000Z");
    const orphanCustomerId = 999999;
    ...
    expect(results).toHaveLength(1);
    expect(results[0].order_id).toBe(staleOrderId);
    expect(results[0].customer_id).toBe(orphanCustomerId);
    expect(results[0].customer_row_id).toBeNull();
    expect(results[0].first_name).toBeNull();
