...
## Defect → Spec table (row 2)

| # | Defect / gap surfaced | Spec patch |
|---|---|---|
| 2 | `send_alert` (MCP) and `alert-check.ts` shared one dedupe namespace `(order_id, calendar_day)` in the same outbox — an unrelated manual alert for an order could silently suppress that day's automated stale-order alert for the same order, or vice versa. Not something either spec explicitly addressed (the stale-order-alerts spec deliberately deferred the MCP tool's contract to "its own spec," per the M1 interview's Q12). Surfaced by asking directly rather than assuming the shared key was intentional. | Added a `source` field to the outbox's internal dedupe key: `alert-check.ts`'s automated alerts stay unset/default-bucketed (matching spec §2.1's exact 6-field `AlertLine` shape unchanged), `send_alert`'s manual alerts get `source: "mcp-send-alert"` — separating the two dedupe namespaces so they can no longer collide. |

## M4 — End-to-end verification (live, agent-driven)

Run in a fresh session (hooks + MCP both loaded clean):

1. Deleted `ecommerce.db` (didn't exist), ran `npm run seed:demo` → seeded stale pending orders 1 (4 days old) and 2 (10 days old), fresh pending order 3 (1 day old), shipped order 4 (20 days old, excluded by status).
2. `npm run alert:check` → exit 0, delivered alerts for orders 2 and 1 (oldest processed first).
3. `list_sent_alerts` (MCP) → confirmed exactly 2 alerts, `order_id` 1 and 2, both `calendar_day: "2026-07-19"`.
4. Re-ran `npm run alert:check` → both logged `skipped-duplicate`, exit 0; `list_sent_alerts` confirmed still exactly 2 entries — no duplicates added.
5. Manual delivery: `send_alert` for order 999, dedupe key `manual-test-1` → confirmed via `list_sent_alerts`, landed with `source: "mcp-send-alert"` alongside the two automated entries (which have no `source` field) — proving the dedupe-namespace fix above actually works, not just compiles.

**Failure drill:** `ALERT_OUTBOX="./ecommerce.db/alerts.jsonl" npm run alert:check` (an impossible path — `ecommerce.db` is a file, not a directory, on every OS) → exit 1, one structured error log line (`{"outcome":"error","order_id":2,"calendar_day":"2026-07-19","reason":"EEXIST: file already exists, mkdir '...'"}`), and the real `outbox/alerts.jsonl` confirmed untouched (still exactly 3 lines) afterward.

Explicitly verified (not assumed) that this failure aborts the whole run rather than falling through to AC7's per-order-continue behavior: built a temporary synthetic test mocking `deliverAlert` to throw only on its first call with two seeded stale orders — `deliverAlert` was called exactly once, exactly one error line was logged, and the second order's delivery was never attempted at all. Confirmed this matches the code (`break` after the first infra-level throw, never reaching AC7's `continue` path) — then added this as a permanent regression test rather than a one-off manual check.

**M4 status: implementation done.** All ACs covered by tests, e2e run proves the feature end-to-end including idempotency, manual MCP delivery, and the failure path with no partial writes.

## Steering failures

None — every source-code change across all four milestones was made by the agent (Write/Edit tool calls), never by hand. The closest thing to a "failure" was a repeated coaching pattern rather than a hand-edit: in M3, the agent twice self-corrected a requested rule violation (relocating a query instead of writing it where I'd asked, to test the scope guard) before the guard was ever actually invoked. That's arguably the opposite of a steering failure — a well-aligned agent following the rules voluntarily — but it meant I had to explicitly instruct it *not* to work around the rule and attempt the literal violation before I got real evidence the hook worked. Same pattern recurred with `green_gate.js`/`audit_log.js` in M3: initial verification was synthetic (direct stdin to the hook script) rather than a live triggered event, and needed a second, forced pass to count as real evidence.

## Defect → Spec table (final count: 2, meets the ≥2 requirement)

See the two rows above: (1) the outbox-directory auto-create gap found by the M1 agent-test, (2) the `send_alert`/`alert-check` dedupe-namespace collision found by asking a direct design question after the MCP server was built.

## Closing reflection

The spec-first discipline paid for itself almost immediately once implementation started — nearly every "gotcha" that would normally surface as a runtime bug during M4 (the outbox directory not existing, the AC6/AC7 failure-mode split, the dedupe-namespace collision between two callers) had either already been decided in M1's interview or got caught by an explicit question before it shipped, rather than discovered later by a broken demo. The single most valuable habit across all four milestones was the same one, applied at every layer: don't trust that "the agent said it works" — force the actual adversarial case (the literal rule violation for hooks, the cold agent-test for the spec, the synthetic-then-live failure injection for the gates) and watch it fail or block for real before believing it. Every milestone had at least one moment where a synthetic or self-reported "pass" turned out to be unearned until it was forced to happen live.
